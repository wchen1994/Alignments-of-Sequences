import alignSeq.project4 as project4
...
def test_compute_local_alignment():
    ...
    result = project4.compute_local_alignment('', '', score_matrix, [[0]])
    assert result == (0,'','')

    result = project4.compute_local_alignment('A','A',score_matrix, [[0,0],[0,6]])
    assert result == (6,'A','A')

    seq_x = 'TTTTTATGTTTTT'
    seq_y = 'ACG'
    align_matrix = project4.compute_alignment_matrix(seq_x,seq_y,score_matrix, False)
    result = project4.compute_local_alignment(seq_x,seq_y,score_matrix,align_matrix)
    assert result == (14,'ATG','ACG')
 
    alphabet = set('abcdefghijklmnopqrstuvwxyz')
    score_matrix = project4.build_scoring_matrix(alphabet, 2, -1, -1)

    seq_x = 'racecar'
    seq_y = 'longracecartrack'
    align_matrix = project4.compute_alignment_matrix(seq_x, seq_y, score_matrix, False)
    result = project4.compute_local_alignment(seq_x, seq_y, score_matrix, align_matrix)
    assert result == (14,'racecar','racecar')
